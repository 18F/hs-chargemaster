...
then
  python manage.py migrate --settings=chargemaster.settings.production --noinput
else
...
python manage.py collectstatic --settings=chargemaster.settings.production --noinput
gunicorn -t 120 -k gevent -w 2 chargemaster.wsgi:application
